...
class SomunicateApp:
            ...
            st.markdown("""
                ...
                <div class='custom-container'>
                    <ol class='custom-bullet'>
                        ...
                        <li><strong>Receive Your Match:</strong> This algorithm identifies the top 1-10 closest sounds to your ratings using a Mahalanobis distance calculation:
                            <ul>
    ...
    def get_user_group_ids(self):
        ...
        for param in groupID_params:
            ...
            ##########Variable for Summary Demographic Display###############
            DEMOGRAPHIC_DISPLAY.append(selected_params[param])
    ...
    def display_results(self, user_ratings, selected_dimensions, top_n, data, group_id):
        ...
        if len(user_ratings) > 0:
            ...
            with col1:
                st.info("Sounds Displayed Below: ")
            ...
            # Combine the elements in DEMOGRAPHIC_DISPLAY into a single list and replace age group keys with their corresponding values
            demographic_display_list = []
            for sublist in DEMOGRAPHIC_DISPLAY:
                for item in sublist:
                    if item in age_range_key:
                        demographic_display_list.append(age_range_key[item])
                    else:
                        demographic_display_list.append(item)

            # Join the elements in the demographic display list with commas
            formatted_demographic_display = ", ".join(demographic_display_list)

            # Combine the selected dimensions into a single string
            dimensions_list = [self.dimension_display[dimension] for dimension in selected_dimensions]
            formatted_dimensions = ", ".join(dimensions_list)

            # Display the formatted demographic and dimensions
            with col1:
                st.markdown(
                    f"<p style='font-size:12px; color:#B39DDB;'>Sounds Tailored to Following Demographic: {formatted_demographic_display}</p>",
                    unsafe_allow_html=True
                )
                st.markdown(
                    f"<p style='font-size:12px; color:#B39DDB;'>Filtering Sounds Based on Following Dimensions: {formatted_dimensions}</p>",
                    unsafe_allow_html=True
                )
                ...
                if os.path.exists(sound_file_path):
                    ...
                    with col1:
                        ...
                        <div style="
                            ...
                            <i><b><strong> Sound: </strong> {sound_file_name}<br>
